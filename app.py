...
from export_utils import (export_to_advanced_docx, export_report_pdf,
                          export_visualization_png, generate_all_process_images, 
                          export_comprehensive_package, DOCX_AVAILABLE) # <-- Ditambah export_comprehensive_package dan generate_all_process_images dari export_utils
...
def display_core_analysis(original_pil, results):
    ...
    # Container utama untuk layout yang stabil
    ...
    with main_container:
        ...
        with col1:
            ...
            # Detail Gambar dengan expander yang stabil
            detail_expander = st.expander("Detail Gambar", expanded=False)
    ...
    display_single_image(
        title="Enhanced Error Level Analysis (ELA)", image_array=ela_image_display, cmap='hot',
        caption="Area yang lebih terang menunjukkan potensi tingkat kompresi yang berbeda. Analisis telah ditingkatkan dengan deteksi tekstur dan multi-skala.",
        details=f"- **Mean ELA:** `{results.get('ela_mean', 0):.2f}`\n- **Std Dev ELA:** `{results.get('ela_std', 0):.2f}`\n- **Region Outlier:** `{results.get('ela_regional_stats', {}).get('outlier_regions', 0)}`\n- **Suspicious Regions:** `{len(results.get('ela_regional_stats', {}).get('suspicious_regions', []))}`\n- **Texture-Aware Score:** `{results.get('ela_regional_stats', {}).get('texture_aware_score', 0):.3f}`\n- **Enhancement Ratio:** `{results.get('ela_regional_stats', {}).get('signal_enhancement_ratio', 1):.2f}x`",
...
def display_advanced_analysis(original_pil, results):
    ...
    st.write("Tahap ini menyelidiki properti intrinsik gambar seperti frekuensi, tekstur, tepi, artefak kompresi, dan deteksi pemalsuan gambar.")
    
    # Baris 1: Analisis Dasar
    ...
    # Baris 2: Analisis Iluminasi dan Deteksi Pemalsuan
    ...
    # MM Fusion Forgery Detection
    with col5:
        st.subheader("MM Fusion: Deteksi Pemalsuan Multi-Modal", divider='rainbow')
        
        # Check if MM Fusion analysis exists
        mm_fusion_results = results.get('mm_fusion_analysis', {})
        
        if mm_fusion_results and 'error' not in mm_fusion_results:
            ...
            heatmap_data = mm_fusion_results.get('heatmap_data', {})
            if isinstance(heatmap_data, dict) and 'heatmap_array' in heatmap_data:
                heatmap_array = np.array(heatmap_data['heatmap_array'])
                if heatmap_array.size > 0:
                    # Resize heatmap to match original image if needed
                    if heatmap_array.shape != original_array.shape[:2]:
                        heatmap_array = cv2.resize(heatmap_array, (original_array.shape[1], original_array.shape[0]))
                    ax1.imshow(heatmap_array, cmap='hot', alpha=0.5)
            ax1.set_title('Original + Heatmap Deteksi')
            ax1.axis('off')
            
            # Right: Detailed metrics
            ax2.axis('off')
            
            # Display key metrics from updated structure
            forgery_detected = mm_fusion_results.get('forgery_detected', False)
            forgery_confidence = mm_fusion_results.get('forgery_confidence_score', 0)
            confidence_level = mm_fusion_results.get('confidence_level', 'LOW')
            confidence_factors = mm_fusion_results.get('confidence_factors', [])
            
            # Color coding for confidence
            if forgery_confidence > 70:
                conf_color = 'red'
                assessment = 'Tinggi'
            elif forgery_confidence > 40:
                conf_color = 'orange'
                assessment = 'Sedang'
            else:
                conf_color = 'green'
            ...
            ax2.text(0.1, 0.9, 'METRIK MM FUSION:', transform=ax2.transAxes, fontsize=12, weight='bold')
            ax2.text(0.1, 0.8, f'Keandalan Deteksi: {confidence_level}', transform=ax2.transAxes, fontsize=10)
            ax2.text(0.1, 0.7, f'Pemalsuan Terdeteksi: {"Ya" if forgery_detected else "Tidak"}', transform=ax2.transAxes, fontsize=10)
            ax2.text(0.1, 0.6, f'Confidence: {forgery_confidence:.1f}% ({assessment})', 
                    transform=ax2.transAxes, fontsize=10, color=conf_color, weight='bold')
            
            # Add confidence factors if available
            if confidence_factors:
                ax2.text(0.1, 0.5, 'Faktor Kontribusi:', transform=ax2.transAxes, fontsize=10, weight='bold')
                y_pos = 0.45
                for i, (factor_name, factor_value) in enumerate(confidence_factors[:3]):  # Show top 3
                    ax2.text(0.1, y_pos - i*0.05, f'  • {factor_name}: {factor_value:.1f}%', 
            ...
            with st.expander("Detail Teknis MM Fusion", expanded=False):
                technical_details = mm_fusion_results.get('technical_details', {})
                st.markdown(f"""
**Metodologi MM Fusion:**
- {technical_details.get('methodology', 'Multi-modal fusion analysis')}
- Reliabilitas: {technical_details.get('reliability_rating', 'High')}
- Threshold deteksi: {technical_details.get('detection_threshold', '25%')}

**Parameter Teknis:**
- **Confidence Score:** `{forgery_confidence:.1f}%`
- **Tingkat Keandalan:** `{confidence_level}`
- **Pemalsuan Terdeteksi:** `{"Ya" if forgery_detected else "Tidak"}`
- **Metode Deteksi:** Multi-layer fusion analysis

**Parameter yang Diukur:**
{chr(10).join([f"- {param}" for param in technical_details.get('parameters_measured', [])][:5])}
...
""")
            ...
            st.info("Analisis MM Fusion tidak tersedia. Jalankan analisis lengkap untuk mengaktifkan fitur ini.")
    
    # TruFor Forgery Detection
    with col6:
        st.subheader("TruFor: Forensic Analysis", divider='rainbow')
        
        # Check if TruFor analysis exists
        trufor_results = results.get('trufor_analysis', {})
        
        if trufor_results and 'error' not in trufor_results:
            # Create visualization
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
            
            # Left: Original image with forgery localization overlay
            original_array = np.array(original_pil)
            ax1.imshow(original_array)
            
            # Overlay forgery localization map if available
            forgery_map = trufor_results.get('forgery_localization_map')
            if forgery_map is not None and isinstance(forgery_map, np.ndarray) and forgery_map.size > 0:
                # Resize forgery map to match original image if needed
                if forgery_map.shape != original_array.shape[:2]:
                    forgery_map = cv2.resize(forgery_map, (original_array.shape[1], original_array.shape[0]))
                
                # Apply colormap and overlay
                # Use 'jet' colormap for better visibility (blue=safe, red=suspicious)
                im = ax1.imshow(forgery_map, cmap='jet', alpha=0.5, vmin=0, vmax=1)
                ax1.set_title('Peta Lokalisasi Pemalsuan')
                
                # Add colorbar
                cbar = plt.colorbar(im, ax=ax1, fraction=0.046, pad=0.04)
                cbar.set_label('Tingkat Kecurigaan', rotation=270, labelpad=15)
            else:
                ax1.set_title('Analisis Multi-Skala')
            ax1.axis('off')
            ...
            # Display key metrics from updated structure
            forensic_confidence = trufor_results.get('forensic_confidence', 0)
            is_authentic = trufor_results.get('is_authentic', False)
            risk_level = trufor_results.get('risk_level', 'UNKNOWN')
            forensic_report = trufor_results.get('forensic_report', {})
            ...
            ax2.text(0.1, 0.9, 'METRIK TRUFOR:', transform=ax2.transAxes, fontsize=12, weight='bold')
            ax2.text(0.1, 0.8, f'Status: {"Autentik" if is_authentic else "Mencurigakan"}', 
                    transform=ax2.transAxes, fontsize=10, color='green' if is_authentic else 'red')
            ax2.text(0.1, 0.7, f'Forensic Confidence: {forensic_confidence:.1f}%', transform=ax2.transAxes, fontsize=10)
            ax2.text(0.1, 0.6, f'Risk Level: {risk_level} ({assessment})', 
                    transform=ax2.transAxes, fontsize=10, color=score_color, weight='bold')
            
            # Add forgery map statistics if available
            forgery_stats = trufor_results.get('forgery_map_stats', {})
            if forgery_stats:
                suspicious_area = forgery_stats.get('suspicious_area_percentage', 0)
                ax2.text(0.1, 0.5, f'Area Mencurigakan: {suspicious_area:.1f}%', 
                        transform=ax2.transAxes, fontsize=10, 
                        color='red' if suspicious_area > 30 else 'orange' if suspicious_area > 10 else 'green')
            
            # Add key findings if available
            if forensic_report and 'key_findings' in forensic_report:
                ax2.text(0.1, 0.4, 'Temuan Utama:', transform=ax2.transAxes, fontsize=10, weight='bold')
                y_pos = 0.35
                for i, finding in enumerate(forensic_report['key_findings'][:2]):  # Show top 2
                    # Wrap long text
                    if len(finding) > 40:
                        finding = finding[:40] + '...'
                    ax2.text(0.1, y_pos - i*0.05, f'  • {finding}', 
            ...
            # Technical details expander
            with st.expander("Detail Teknis TruFor", expanded=False):
                technical_details = trufor_results.get('technical_details', {})
                st.markdown(f"""
**Metodologi TruFor:**
- {technical_details.get('methodology', 'Multi-scale forensic analysis')}
- Reliabilitas: {technical_details.get('reliability_rating', 'Very High')}
- Kedalaman analisis: {technical_details.get('analysis_depth', 'Deep')}  

**Parameter Teknis:**
- **Forensic Confidence:** `{forensic_confidence:.1f}%`
- **Status Autentik:** `{"Ya" if is_authentic else "Tidak"}`
- **Risk Level:** `{risk_level}`
- **Threshold Autentikasi:** {technical_details.get('authentication_threshold', '70%')}

**Parameter Analisis:**
{chr(10).join([f"- {param}" for param in technical_details.get('analysis_parameters', [])][:5])}
...
**Rekomendasi:**
{chr(10).join([f"- {rec}" for rec in forensic_report.get('recommendations', [])][:2]) if forensic_report else "Tidak ada rekomendasi tersedia"}
""")
        else:
            st.info("Analisis TruFor tidak tersedia. Jalankan analisis lengkap untuk mengaktifkan fitur ini.")
    
    # Enhanced JPEG Ghost visualization
    ...
    with col5:
        st.subheader("Analisis JPEG Ghost", divider='rainbow')
        
        # Create enhanced visualization
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
        
        # Left plot: JPEG Ghost heatmap
        if isinstance(jpeg_ghost_display, np.ndarray) and jpeg_ghost_display.size > 0:
            # Ensure the ghost map is properly scaled
            if jpeg_ghost_display.dtype != np.uint8:
                if np.max(jpeg_ghost_display) <= 1.0:
                    # Scale from 0-1 to 0-255
                    ghost_map_display = (jpeg_ghost_display * 255).astype(np.uint8)
                else:
                    ghost_map_display = jpeg_ghost_display.astype(np.uint8)
            else:
                ghost_map_display = jpeg_ghost_display
                
            # Apply colormap for better visualization
            im1 = ax1.imshow(ghost_map_display, cmap='hot', interpolation='bilinear')
            ax1.set_title('JPEG Ghost Detection Map')
            ax1.axis('off')
            
            # Add colorbar
            cbar1 = fig.colorbar(im1, ax=ax1, fraction=0.046, pad=0.04)
            cbar1.set_label('Ghost Intensity', rotation=270, labelpad=15)
            
            # Add text overlay for key metrics
            ghost_score = ghost_analysis.get('total_ghost_score', 0)
            coverage = ghost_analysis.get('ghost_coverage', 0)
            
            # Color code the score
            if ghost_score > 0.5:
                score_color = 'red'
                assessment = 'High'
            elif ghost_score > 0.2:
                score_color = 'orange'
                assessment = 'Medium'
            else:
                score_color = 'green'
            ...
            ax1.text(0.02, 0.98, f'Ghost Score: {ghost_score:.3f} ({assessment})',
                    transform=ax1.transAxes, fontsize=10, 
                    bbox=dict(boxstyle='round', facecolor='white', alpha=0.8),
                    verticalalignment='top', color=score_color, weight='bold')
        else:
            # Fallback visualization if no ghost map
            ax1.text(0.5, 0.5, 'No Ghost Map Available', 
                    ha='center', va='center', transform=ax1.transAxes)
            ax1.axis('off')
        
        # Right plot: Quality response analysis
        quality_analysis = ghost_analysis.get('quality_analysis', {})
        if quality_analysis:
            ...
            for q_str, q_data in quality_analysis.items():
                try:
                    q = int(q_str)
                    qualities.append(q)
                    mean_responses.append(q_data.get('mean_response', 0))
                    low_response_areas.append(q_data.get('low_response_area', 0))
            ...
            if qualities:
                ...
                mean_responses = [mean_responses[i] for i in sorted_indices]
                low_response_areas = [low_response_areas[i] for i in sorted_indices]
            
            # Only plot if we have data
            if qualities and mean_responses:
                # Create dual y-axis plot
                ax2_right = ax2.twinx()
                
                # Plot mean response
                line1 = ax2.plot(qualities, mean_responses, 'b-o', label='Mean Response', markersize=4)
                ax2.set_xlabel('JPEG Quality')
                ax2.set_ylabel('Mean Response', color='b')
                ax2.tick_params(axis='y', labelcolor='b')
                
                # Plot low response area percentage
                line2 = ax2_right.plot(qualities, low_response_areas, 'r-s', label='Low Response Area', markersize=4)
                ax2_right.set_ylabel('Low Response Area Ratio', color='r')
                ax2_right.tick_params(axis='y', labelcolor='r')
                
                # Find and mark suspicious qualities
                suspicious_qualities = [q for q, area in zip(qualities, low_response_areas) if area > 0.2]
                if suspicious_qualities:
                    for sq in suspicious_qualities:
                        ax2.axvline(x=sq, color='orange', linestyle='--', alpha=0.5)
                        if mean_responses:  # Check if we have responses to avoid division by zero
                            ax2.text(sq, max(mean_responses)*0.9, f'{sq}', 
                                    ha='center', fontsize=8, color='orange')
                
                ax2.set_title('Quality Response Analysis')
                ax2.grid(True, alpha=0.3)
                
                # Combine legends
                lines = line1 + line2
                labels = [l.get_label() for l in lines]
                ax2.legend(lines, labels, loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=2)
            else:
                # No data available, show placeholder
                ax2.text(0.5, 0.5, 'No Quality Analysis Data Available', 
            ...
            ax2.text(0.5, 0.5, 'No Quality Analysis Available', 
                    ha='center', va='center', transform=ax2.transAxes)
            ax2.axis('off')
        ...
        st.caption("Area terang dalam peta menunjukkan kemungkinan kompresi JPEG ganda. "
                  "Grafik kanan menunjukkan respons gambar terhadap berbagai kualitas JPEG.")
        
        # Detail Teknis dengan expander yang stabil
        ghost_technical_expander = st.expander("Lihat Detail Teknis", expanded=False)
        with ghost_technical_expander:
            st.markdown(f"""**Analisis JPEG Ghost:**
- **Rasio Area Mencurigakan:** `{results.get('jpeg_ghost_suspicious_ratio', 0):.2%}`
- **Ghost Coverage:** `{ghost_analysis.get('ghost_coverage', 0):.2%}`
- **Ghost Intensity:** `{ghost_analysis.get('ghost_intensity', 0):.3f}`
- **Total Ghost Score:** `{ghost_analysis.get('total_ghost_score', 0):.3f}`
- **Jumlah Region Ghost:** `{len(ghost_analysis.get('ghost_regions', []))}`

**Interpretasi:**
- Ghost Score > 0.5: Kemungkinan tinggi kompresi ganda
- Ghost Score 0.2-0.5: Kemungkinan sedang kompresi ganda
- Ghost Score < 0.2: Kemungkinan rendah kompresi ganda

**Region Ghost Terdeteksi:**
""")
            ghost_regions = ghost_analysis.get('ghost_regions', [])
            if ghost_regions:
                for i, region in enumerate(ghost_regions[:5]):  # Show top 5 regions
                    st.markdown(f"- Region {i+1}: Size={region['size']} pixels, "
                              f"Strength={region['ghost_strength']:.3f}, "
                              f"Est. Quality={region.get('estimated_quality', 'N/A')}")
            else:
                st.markdown("- Tidak ada region ghost signifikan yang terdeteksi")
    with col6:
...
def display_statistical_analysis(original_pil, results):
    ...
    # PERBAIKAN PATH AKSES DATA DI SINI
    display_single_plot(
        title="Kurva Respons Kualitas JPEG",
        plot_function=create_quality_response_plot,
        args=[results],
        caption="Error saat gambar dikompres ulang pada kualitas berbeda.",
        details=f"- **Estimasi Kualitas Asli:** `{results.get('jpeg_analysis', {}).get('basic_analysis', {}).get('estimated_original_quality', 'N/A')}`",
        container=col2
    )
    # AKHIR PERBAIKAN
...
def display_final_report(results):
    """Fungsi yang menampilkan Laporan Akhir (Tahap 4) dengan terminologi yang diperbarui."""
    st.header("Tahap 4: Laporan Akhir dan Interpretasi Forensik")
    st.markdown(
        "Bagian ini merangkum temuan dari 17 tahap analisis, menyajikan penilaian probabilitas manipulasi, "
        "dan mengevaluasi stabilitas hasil analisis berdasarkan koherensi bukti-bukti forensik."
    )
    st.markdown("---")

    # Panel penjelasan tetap sama
    with st.expander("ℹ️ Cara Membaca Laporan Ini"):
        st.markdown("""
        - **Asesmen Utama:** Kesimpulan paling mungkin dari sistem berdasarkan totalitas bukti.
        - **Reliabilitas Penilaian:** Menunjukkan seberapa andal kesimpulan utama tersebut. Nilai 'Tinggi' atau 'Sangat Tinggi' berarti bukti sangat konsisten.
        - **Distribusi Probabilitas:** Menunjukkan probabilitas untuk setiap jenis kesimpulan (Asli, Copy-Move, Splicing). Interval kepercayaan (garis hitam) menunjukkan rentang kemungkinan nilai sebenarnya.
        - **Koherensi Indikator Forensik:** Menganalisis seberapa 'sepakat' berbagai metode deteksi satu sama lain. Koherensi tinggi meningkatkan keandalan hasil.
        - **Rekomendasi:** Saran langkah selanjutnya berdasarkan hasil analisis.
        """)
    ...
    # PERBAIKAN: Gunakan 'report' dari 'uncertainty_analysis'
    report_details = uncertainty_analysis.get('report', {})
    probabilities = uncertainty_analysis.get('probabilities', {})
    
    result_container = st.container(border=True)
    with result_container:
        if report_details and probabilities:
            # Mengambil data dari `report_details` yang sudah diperbarui
            primary_assessment = report_details.get('primary_assessment', 'N/A')
            # Hapus prefix "Indikasi: " jika ada
            if primary_assessment.startswith('Indikasi: '):
                primary_assessment = primary_assessment.replace('Indikasi: ', '')
            assessment_reliability = report_details.get('assessment_reliability', 'N/A')
            uncertainty_level = probabilities.get('uncertainty_level', 0)
            indicator_coherence = report_details.get('indicator_coherence', 'N/A')

            st.subheader("Ringkasan Eksekutif Analisis", divider="rainbow")
            col1, col2 = st.columns([2, 1])

            with col1:
                # Menampilkan Asesmen Utama
                if "manipulasi" in primary_assessment.lower() or "forgery" in primary_assessment.lower():
                    st.warning(f"**Asesmen Utama:** {primary_assessment}", icon="⚠️")
                else:
                    st.success(f"**Asesmen Utama:** {primary_assessment}", icon="✅")

                # Menampilkan Reliabilitas Penilaian (Bukan lagi "Skor Kepercayaan")
                # Jika tidak terlalu ambigu, tampilkan tingkat reliabilitasnya
                if uncertainty_level < 0.35: # Ambang batas ketidakpastian
                     st.metric(label="Reliabilitas Penilaian", value=assessment_reliability)
                else:
                    # Jika sangat tidak pasti, tunjukkan level ketidakpastian sebagai gantinya
                    st.metric(label="Tingkat Ketidakpastian", value=f"{uncertainty_level*100:.1f}%",
                              delta="Hasil Tidak Stabil/Ambigu", delta_color="inverse")

                # Progress bar untuk probabilitas hasil utama
                main_prob_key = "authentic_probability" # Default
                if "copy-move" in primary_assessment.lower(): main_prob_key = "copy_move_probability"
                elif "splicing" in primary_assessment.lower(): main_prob_key = "splicing_probability"
                
                main_prob_value = probabilities.get(main_prob_key, 0)
                st.progress(main_prob_value, text=f"Probabilitas Hasil Utama: {main_prob_value:.1%}")
                
                # Tidak perlu menampilkan skor lama (copy-move dan splicing) di sistem baru

                # Rekomendasi Tindak Lanjut dengan expander yang stabil
                recommendation_expander = st.expander("Rekomendasi Tindak Lanjut", expanded=False)
                with recommendation_expander:
                    st.info(report_details.get('recommendation', 'Tidak ada rekomendasi tersedia.'))

            with col2:
                # Spider chart tetap relevan untuk menunjukkan kontribusi metode
                st.plotly_chart(create_spider_chart(results), use_container_width=True)

            # Bagian visualisasi probabilitas dan ketidakpastian
            st.subheader("Analisis Probabilitas dan Ketidakpastian", divider='blue')

            # Tampilkan Koherensi Indikator Forensik
            st.info(f"**Evaluasi Koherensi Bukti:** {indicator_coherence}")
            
            fig_prob, (ax1_prob, ax2_prob) = plt.subplots(1, 2, figsize=(16, 5))
            
            # Grafik Bar Probabilitas dengan Interval Kepercayaan
            create_probability_bars(ax1_prob, results)
            
            # Visualisasi Detail Ketidakpastian
            create_uncertainty_visualization(ax2_prob, results) 
            
            st.pyplot(fig_prob)
            
            # Indikator Keandalan Teknis dengan expander yang stabil
            reliability_expander = st.expander("🔍 Lihat Detail Indikator Keandalan Teknis", expanded=False)
            with reliability_expander:
                ...
                if reliability_indicators:
                    ...
                    st.write("Tidak ada indikator spesifik yang ditandai.")

        else:
            st.error("Gagal memuat laporan analisis ketidakpastian. Data mungkin tidak lengkap atau terjadi kesalahan.", icon="❌")
            # Tampilkan fallback jika ada (misal, dari klasifikasi lama)
            old_type = classification.get('type', "N/A")
            old_confidence = classification.get('confidence', 'N/A')
            st.metric(label=f"Hasil Klasifikasi (Fallback): {old_type}", value=old_confidence)
    ...
    for idx, entry in enumerate(reversed(history_data)):
        ...
        with st.container():
            ...
            with col_chk:
                ...
                is_selected = st.checkbox("Pilih", key=f"select_{original_entry_idx}", value=original_entry_idx in st.session_state.selected_history, label_visibility="hidden")
                ...
                with st.expander(expander_title):
                    ...
                    with col1_detail:
                        ...
                        if thumbnail_path and os.path.exists(thumbnail_path):
                            st.image(thumbnail_path, width=300)  # Fixed: changed 'stretch' to numeric width
                        else:
                            st.caption("Thumbnail tidak tersedia.")
                    
                    with col2_detail:
                        # Menampilkan data uncertainty jika tersedia
                        uncertainty_level = summary.get('uncertainty_level', None)
                        indicator_coherence = summary.get('indicator_coherence', None)
                        
                        st.markdown(f"**Reliabilitas Penilaian:** {summary.get('confidence', 'N/A')}")
                        
                        # Tampilkan uncertainty level jika ada
                        if uncertainty_level is not None:
                            if uncertainty_level < 0.35:
                                uncertainty_color = "green"
                                uncertainty_text = "Rendah"
                            elif uncertainty_level < 0.65:
                                uncertainty_color = "orange"
                                uncertainty_text = "Sedang"
                            else:
                                uncertainty_color = "red"
                                uncertainty_text = "Tinggi"
                            st.markdown(f"**Tingkat Ketidakpastian:** <span style='color:{uncertainty_color}'>{uncertainty_level:.1%} ({uncertainty_text})</span>", unsafe_allow_html=True)
                        
                        # Tampilkan koherensi indikator jika ada
                        if indicator_coherence:
                            st.markdown(f"**Koherensi Indikator:** {indicator_coherence}")
...
def validate_pipeline_integrity(analysis_results):
    ...
    pipeline_processes = [
        ...
        {"name": "5. Ekstraksi Fitur (SIFT, ORB, etc.)", "check": lambda r: isinstance(r.get('feature_sets'), dict) and 'sift' in r.get('feature_sets', {}) and isinstance(r['feature_sets'].get('sift'), tuple) and len(r['feature_sets']['sift']) >= 2 and r['feature_sets']['sift'][0] is not None},
...
def display_validation_tab_baru(analysis_results):
    ...
    with col2:
        ...
        # Show ELA analysis details
        with st.expander("📊 Detail Analisis ELA", expanded=False):
            ela_mean = analysis_results.get('ela_mean', 0)
            ...
            with col_ela1:
                st.metric("ELA Mean", f"{ela_mean:.2f}")
                st.metric("ELA Std Dev", f"{ela_std:.2f}")
                st.metric("Confidence Score", f"{ela_confidence*100:.1f}%")
            
            with col_ela2:
                st.metric("Outlier Regions", regional_stats.get('outlier_regions', 0))
                st.metric("Regional Inconsistency", f"{regional_stats.get('regional_inconsistency', 0):.3f}")
                st.metric("Suspicious Regions", len(regional_stats.get('suspicious_regions', [])))
            
            st.markdown("---")
            st.markdown("**Detail Analisis:**")
            st.markdown(ela_details)
...
def main_app():
    st.set_page_config(layout="wide", page_title="foreinsight Image")
    ...
    # Header dengan logo dan judul - sidebar (responsif)
    st.sidebar.markdown("""
    <div style='text-align: center; margin-bottom: 20px; padding: 10px;'>
        <div style='display: flex; flex-direction: column; align-items: center; gap: 10px;'>
    """, unsafe_allow_html=True)
    
    # Logo dengan container responsif
    try:
        col1, col2, col3 = st.sidebar.columns([1, 2, 1])
        with col2:
            st.image("Logo-UHO-Normal-1-294x300[1].png", width=80)
    except:
        st.sidebar.markdown("<div style='font-size: 3em; margin: 10px 0;'>🏛️</div>", unsafe_allow_html=True)
    
    st.sidebar.markdown("""
            <h3 style='margin: 5px 0 15px 0; color: #1f77b4; font-size: 1.2em; font-weight: 600;'>foreinsight Image</h3>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.original_image:
        ...
        if st.sidebar.button("🔬 Mulai Analisis", use_container_width=True, type="primary"):
            st.session_state.analysis_results = None
            # Use appropriate temp directory for the current OS
            if os.name == 'nt':  # Windows
                temp_base = os.environ.get('TEMP', os.getcwd())
                temp_dir = os.path.join(temp_base, 'temp_uploads')
            else:  # Unix/Linux/Mac
                temp_dir = "/tmp/temp_uploads" if os.path.exists("/tmp") else "temp_uploads"
...
# Tombol Keluar (tidak ada perubahan pada logika ini)
    ...
    # Header utama sederhana tanpa logo
    st.markdown("""
    <div style='text-align: center; margin-bottom: 20px;'>
        <p style='margin: 0; color: #666; font-size: 0.85em; font-weight: 400; line-height: 1.4;'>
            Universitas Halu Oleo<br>
            <span style='color: #888; font-size: 0.9em;'>Fakultas Teknik</span>
        </p>
        <h2 style='margin: 10px 0; color: #6c757d; font-weight: 400; font-size: 1.5em;'>Sistem Deteksi Manipulasi Gambar Digital</h2>
        <div style='width: 60%; height: 2px; background: linear-gradient(90deg, #1f77b4, #17a2b8); margin: 15px auto; border-radius: 1px;'></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.analysis_results:
        ...
        with tab6:
            ...
            export_format = st.selectbox(
                ...
                help="Pilih format ekspor yang diinginkan. Untuk PDF, pastikan LibreOffice terinstal atau gunakan opsi DOCX."
            )
            
            # Informasi bantuan untuk ekspor PDF
            if export_format == "Laporan PDF":
                with st.expander("ℹ️ Informasi Konversi PDF", expanded=False):
                    ...
                    if is_huggingface:
                        st.warning("""
                        **⚠️ Keterbatasan di Hugging Face:**
                        
                        Konversi DOCX ke PDF tidak tersedia di lingkungan Hugging Face Spaces
                        karena keterbatasan akses sistem.
                        
                        **Solusi:**
                        1. Unduh file DOCX menggunakan tombol "Unduh Laporan DOCX"
                        2. Konversi manual ke PDF menggunakan:
                           - Microsoft Word (desktop)
                           - LibreOffice (gratis)
                           - Konverter online
                        
                        Sistem akan memberikan PDF visualisasi sebagai alternatif.
                        """)
                    else:
                        st.info("""
                        **Untuk konversi PDF yang optimal:**
                        
                        1. **Install LibreOffice** (direkomendasikan):
                           - Windows: Download dari [libreoffice.org](https://www.libreoffice.org/download/download/)
                           - Linux: `sudo apt install libreoffice`
                           - macOS: `brew install libreoffice`
                        
                        2. **Atau install docx2pdf**:
                           ```bash
                           pip install docx2pdf
                           ```
                        
                        3. **Alternatif**: Unduh file DOCX dan konversi manual menggunakan Microsoft Word
                        
                        Tanpa dependensi di atas, sistem akan mencoba metode fallback.
                        """)

            if export_format != "-":
                # Use appropriate temp directory for the current OS
                if os.name == 'nt':  # Windows
                    export_dir = os.path.join(os.environ.get('TEMP', 'exported_reports'), 'exported_reports')
                else:  # Unix/Linux/Mac
                    export_dir = "/tmp/exported_reports" if os.path.exists("/tmp") else "exported_reports"
                            ...
                            if export_format == "Laporan DOCX":
                                ...
                                if docx_for_pdf:
                                    ...
                                    if result_path and os.path.exists(result_path):
                                        ...
                                        # Jika konversi PDF gagal, berikan opsi untuk mengunduh DOCX langsung
                                        st.warning("Konversi ke PDF gagal, tetapi file DOCX berhasil dibuat.")
                                        st.info("Anda dapat mengunduh file DOCX dan mengonversinya secara manual menggunakan Microsoft Word atau LibreOffice.")
                                        with open(docx_for_pdf, "rb") as f:
                                            st.download_button("📄 Unduh Laporan DOCX", f, file_name=os.path.basename(docx_for_pdf))
                                    
                                    # Hapus file DOCX sementara
                                    if os.path.exists(docx_path_temp):
                                        os.remove(docx_path_temp)
                                else:
                                ...
                                # Use appropriate temp directory for the current OS
                                if os.name == 'nt':  # Windows
                                    zip_dir = os.path.join(os.environ.get('TEMP', 'exported_reports'), 'exported_reports')
                                else:  # Unix/Linux/Mac
                                    zip_dir = "/tmp/exported_reports" if os.path.exists("/tmp") else "exported_reports"
                                os.makedirs(zip_dir, exist_ok=True)
                                zip_path_base = os.path.join(zip_dir, f"{base_filename}_full_package")
...
if __name__ == '__main__':
    ...
    # Fix untuk Hugging Face Spaces: jalankan Streamlit server programmatically
    import sys
    from streamlit.web import cli as stcli
    
    # Deteksi apakah sedang dijalankan langsung (bukan via streamlit run)
    if "streamlit.runtime.scriptrunner" not in sys.modules:
        # Jika belum dalam konteks streamlit, jalankan server
        sys.argv = [
            "streamlit", 
            "run", 
            __file__, 
        ...
        # Jika sudah dalam konteks streamlit, jalankan app
        main_app()
